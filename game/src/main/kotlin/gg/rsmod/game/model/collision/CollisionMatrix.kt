...
class CollisionMatrix private constructor(
    val length: Int,
    val width: Int,
    private val matrix: ShortArray,
) {
    ...
    fun isBlocked(
        ...
        return when (direction) {
            Direction.NORTH_WEST ->
                hasFlag(x, y, flags[northwest]) ||
                    hasFlag(x, y, flags[north]) ||
                    hasFlag(x, y, flags[west])
            Direction.NORTH -> hasFlag(x, y, flags[north])
            Direction.NORTH_EAST ->
                hasFlag(x, y, flags[northeast]) ||
                    hasFlag(x, y, flags[north]) ||
                    hasFlag(x, y, flags[east])
            Direction.EAST -> hasFlag(x, y, flags[east])
            Direction.SOUTH_EAST ->
                hasFlag(x, y, flags[southeast]) ||
                    hasFlag(x, y, flags[south]) ||
                    hasFlag(x, y, flags[east])
            Direction.SOUTH -> hasFlag(x, y, flags[south])
            Direction.SOUTH_WEST ->
                hasFlag(x, y, flags[southwest]) ||
                    hasFlag(x, y, flags[south]) ||
                    hasFlag(x, y, flags[west])
    ...
    override fun toString(): String =
        MoreObjects.toStringHelper(this).add("matrices", Arrays.toString(matrix)).toString()
    ...
    companion object {
        ...
        fun createMatrices(
            ...
            width: Int,
            length: Int,
        ): Array<CollisionMatrix> =
            Array(count) {
                CollisionMatrix(width, length)
            }
    }
}
