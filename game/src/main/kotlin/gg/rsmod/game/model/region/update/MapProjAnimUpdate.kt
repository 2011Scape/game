...
) : EntityUpdate<Projectile>(type, entity) {
    override fun toMessage(): Message =
        if (entity.targetPawn != null) {
            val targetIndex =
                if (entity.targetPawn.entityType.isNpc) {
                    entity.targetPawn.index + 1
                } else {
                    -(
                        entity.targetPawn.index +
                            1
                    )
                }
            MapProjAnimMessage(
                start = ((entity.tile.chunkOffsetX shl 3) or entity.tile.chunkOffsetZ),
                pawnTargetIndex = targetIndex,
                offsetX = entity.targetTile.x - entity.tile.x,
                offsetZ =
