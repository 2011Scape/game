...
class CollisionFlagMap(
    val flags: Array<IntArray?> = arrayOfNulls(TOTAL_ZONE_COUNT)
) {
     ...
     */
    operator fun get(absoluteX: Int, absoluteZ: Int, level: Int): Int {
     ...
     */
    operator fun set(absoluteX: Int, absoluteZ: Int, level: Int, mask: Int) {
     ...
     */
    fun add(absoluteX: Int, absoluteZ: Int, level: Int, mask: Int) {
    ...
    fun remove(absoluteX: Int, absoluteZ: Int, level: Int, mask: Int) {
     ...
     */
    fun allocateIfAbsent(absoluteX: Int, absoluteZ: Int, level: Int): IntArray {
        val zoneIndex = zoneIndex(absoluteX, absoluteZ, level)
     ...
     * (3202, 3204, [level]) will have the same results as calling it with (3200, 3200, [level]).
     */
    fun deallocateIfPresent(absoluteX: Int, absoluteZ: Int, level: Int) {
        flags[zoneIndex(absoluteX, absoluteZ, level)] = null
    }

    fun isZoneAllocated(absoluteX: Int, absoluteZ: Int, level: Int): Boolean {
        return flags[zoneIndex(absoluteX, absoluteZ, level)] != null
    }

    @Suppress("NOTHING_TO_INLINE")
    inline fun defaultFlag(): Int {
        return DEFAULT_COLLISION_FLAG
    }

    companion object {

        const val DEFAULT_COLLISION_FLAG: Int = -1
