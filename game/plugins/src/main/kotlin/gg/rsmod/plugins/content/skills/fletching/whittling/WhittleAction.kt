...
class WhittleAction(val definitions: DefinitionSet) {
    ...
    suspend fun cut(task: QueueTask, raw: Int, whittleItem: WhittleItem, amount: Int) {
        ...
        val rawName = player.world.definitions.get(ItemDef::class.java, raw).name.lowercase()
        val productName =
            player.world.definitions.get(ItemDef::class.java, whittleItem.product).name.removeSuffix("(u)").trim()
                .lowercase()
