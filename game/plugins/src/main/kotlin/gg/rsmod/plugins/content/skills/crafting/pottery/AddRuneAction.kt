...
object AddRuneAction {
    ...
    suspend fun attachRune(task: QueueTask, item: Int, amount: Int = 1) {
        val player = task.player
        val inventory = player.inventory
        val urn = PotteryData.findPotteryItemByItem(item - 3)!!
        val rune = PotteryData.findPotteryDataByItem(item - 3)!!.runeRequired!!
        val maxAmount = min(amount, if (inventory.getItemCount(urn.fired) < inventory.getItemCount(rune)) inventory.getItemCount(urn.fired) else inventory.getItemCount(rune))

        if (!canAttach(task, urn))
            return
        player.animate(id = urn.addRuneAnimation)
        repeat(maxAmount) {
            if (!inventory.remove(urn.fired, assureFullRemoval = true).hasSucceeded() || !inventory.remove(rune, assureFullRemoval = true).hasSucceeded())
                return
    ...
    private suspend fun canAttach(task: QueueTask, data: PotteryItem): Boolean {
        val player = task.player
        val inventory = player.inventory
