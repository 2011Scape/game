...
val crystalKey = Items.CRYSTAL_KEY
...
on_item_on_item(item1 = tooth, item2 = loop) {
    ...
    player.inventory.add(item = crystalKey)
    player.message("You join the two halves of the key together.")
}

on_item_on_obj(obj = crystalChest, item = crystalKey) {
    val obj = player.getInteractingGameObj()
    player.inventory.remove(crystalKey, 1)
    ...
    player.lockingQueue(TaskPriority.STRONG) {
        ...
        val drop = DropTableFactory.createDropInventory(player, crystalKey, DropTableType.CHEST)
...
table.register(mainTable, crystalKey, type = DropTableType.CHEST)
