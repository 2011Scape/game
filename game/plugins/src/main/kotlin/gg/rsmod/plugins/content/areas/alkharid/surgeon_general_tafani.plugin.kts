...
on_npc_option(npc = Npcs.SURGEON_GENERAL_TAFANI, option = "talk-to") {
    player.queue {
        ...
        if (player.skills.getCurrentLevel(Skills.HITPOINTS) >= 99) {
...
suspend fun mainChatWith99(it: QueueTask, player: Player) {
    when (it.options(
        ...
        FOURTH_OPTION -> {
            ...
            when (it.options("Sorry, that's much too pricey.", "Sure, that's not too expensive for such a magnificent cape.")) {
                FIRST_OPTION -> {
