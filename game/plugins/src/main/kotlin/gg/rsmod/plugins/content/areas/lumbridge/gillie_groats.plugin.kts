...
suspend fun questDialogue(task: QueueTask, player: Player) {
    when (task.options("I'm after some top-quality milk.", "Who are you?", "Can you tell me how to milk a cow?", "I'm fine, thanks.")) {
        1 -> {
            task.chatPlayer("I'm after some top-quality milk.")
            task.chatNpc("Really? Is it for something special?")
            task.chatPlayer("Most certainly! It's for the cook to make",
                                     "a cake for Duke Horacio!")
            task.chatNpc("Wow, it's quite an honour that you'd pick my cows.",
                                  "I'd suggest you get some milk from my prized cow.")
            task.chatPlayer("Which one's that?")
            task.chatNpc("She's on the east side of the field,",
...
suspend fun whoAreYou(task: QueueTask, player: Player) {
    task.chatNpc("My name's Gillie Groats. My father is a farmer",
                          "and I milk the cows for him.")
    task.chatPlayer("Do you have any buckets of milk spare?")
    task.chatNpc("I'm afraid not. We need all of our milk to sell to market,",
                          "but you can milk the cow yourself if you need milk.")
    task.chatPlayer("Thanks.")
    optionsDialogue(task, player)
}

suspend fun milkTutorial(task: QueueTask, player: Player) {
    task.chatNpc("It's very easy. First, you need an empty",
    ...
    task.chatNpc("They are easy to spot - they have a cowbell around their",
                            "neck and are tethered to a post to stop them wandering",
                            "around all over the place. There are a couple in this field.")
    task.chatNpc("Then you just need to use your bucket on the cow and",
                            "you'll get some tasty, nutritious milk.")
    optionsDialogue(task, player)
}

suspend fun optionsDialogue(task: QueueTask, player: Player) {
