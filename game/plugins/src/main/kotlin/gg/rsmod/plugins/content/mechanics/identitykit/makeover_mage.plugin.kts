...
on_npc_option(npc = Npcs.MAKEOVER_MAGE_2676, option = "Talk-to") {
    player.queue {
        chatNpc("Hello there! I am known as the Makeover Mage! I have", "spent many years researching magics that can change", "your physical appearance!")
        chatNpc("I can alter your physical form with my magic, there's", "no fee. Would you like me to perform my magics upon you?")
        when(options("Tell me more about this 'makeover'.", "Sure, I'll have a makeover.", "Cool amulet! Can I have one?", "No thanks.")) {
...
suspend fun makeoverInformation(it: QueueTask) {
    ...
    it.chatNpc("Why, of course! Basically, and I will try and explain", "this so that you will understand it correctly,")
...
suspend fun purchaseAmulet(it: QueueTask) {
    ...
    it.chatNpc("No problem, but please remember that the amulet I will", "sell you is only a copy of my own. It contains no", "magical powers, and as such, will only cost you 100", "coins.")
    if(it.player.inventory.getItemCount(Items.COINS_995) >= 100) {
        when(it.options("Sure, here you go.", "No way! That's far too expensive.")) {
            1 -> {
                ...
                if(it.player.inventory.remove(Items.COINS_995, 100).hasSucceeded()) {
