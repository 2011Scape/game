...
suspend fun questDialogue(it: QueueTask) {
    ...
    if(it.player.inventory.getItemCount(Items.CLAY) >= 6 && it.player.inventory.getItemCount(Items.COPPER_ORE) >= 4 && it.player.inventory.getItemCount(Items.IRON_ORE) >= 2) {
        it.chatPlayer("I have everything you need.")
        it.chatNpc("Many thanks. Pass them here, please. I can spare you", "some coins for your trouble, and please use my anvils", "any time you want.")
...
suspend fun preQuestDialogue(it: QueueTask) {
    ...
    when(it.options("I wanted to use your anvils.", "I want to use your whetstone.", "Mind your own business, shortstuff!", "I was just checking out the landscape.", "What do you make here?")) {
        1 -> {
            it.chatPlayer("I wanted to use your anvils.")
            it.chatNpc("My anvils get enough work with my own use. I make", "pickaxes, and it takes a lot of hard work. If you could", "get me some more materials, then I could let you use", "them.")
            startQuest(it)
        }
        2 -> {
            it.chatPlayer("I wanted to use your whetstone.")
            it.chatNpc("The whetstone is for more advanced smithing, but I", "could let you use it as well as my anvils if you could", "get me some more materials.")
            startQuest(it)
        }
        3 -> {
            it.chatPlayer("Mind your own business, shortstuff!")
            it.chatNpc("How nice to meet someone with such pleasent manners.", "Do come again when you need to shout at someone", "smaller than you!")
        }
        4 -> {
            it.chatPlayer("I was just checking out the landscape.")
            it.chatNpc("Hope you like it. I do enjoy the solitude of my little", "home. If you get the time, please say hi to my friends in", "the Dwarven Mine.")
            when(it.options("Dwarven Mine?", "Will do!")) {
                1 -> {
                    it.chatPlayer("Dwarven Mine?")
                    it.chatNpc("Yep, the entrance is in the side of Ice Mountain just to", "the east of here. They're a friendly bunch. Stop in at", "Nurmof's store and buy one of my pickaxes!")
                }
                2 -> {
        ...
        5 -> {
            ...
            when(it.options("Who's Nurmof?", "Ah, fair enough.")) {
                1 -> {
                    ...
                    it.chatNpc("Nurmof has a store over in the Dwarven Mine. You", "can find the entrance on the side of Ice Mountain to", "the east of here.")
                }
                2 -> {
