...
on_global_item_pickup {
    if(player.getCurrentStage(cooksAssistant) != 1 && player.getInteractingGroundItem().item == Items.SUPER_LARGE_EGG) {
        player.message("You've no reason to pick that up; eggs of that size are only useful for royal cakes.")
...
suspend fun preQuest(task: QueueTask) {
    task.chatNpc("What am I to do?")
    when (task.options("What's wrong?", "You're a cook, why don't you bake me a cake?", "You don't look very happy.", "Nice hat!")) {
        1 -> {
            questStart(task)
        }
        2 -> {
            task.chatPlayer("You're a cook, why don't you bake me a cake?")
            task.chatNpc("*sniff* Don't talk to me about cakes...")
            questStart(task)
        }
        3 -> {
            task.chatPlayer("You don't look very happy.")
            task.chatNpc("No, I'm not. The world is caving in around me - I am", "overcome by the dark feelings of impending doom.")
            when (task.options("What's wrong?", "I'd take the rest of the day off if I were you.")) {
                ...
                2 -> {
                    ...
                    task.chatNpc("My wife gets sea sick, and I have an irrational fear of", "eyepatches. I don't see it working myself.")
        ...
        4 -> {
            ...
            task.chatPlayer("The whole hat, apron, stripey trousers ensemble - it", "works. It makes you look like a real cook.")
            task.chatNpc("I am a real cook! I haven't got time to be chatting", "about Culinary Fashion. I am in desperate need of help!")
            questStart(task)
...
suspend fun questStart(task: QueueTask) {
    ...
    task.chatNpc("Oh dear, oh dear, oh dear, I'm in a terrible terrible", "mess! It's the Duke's birthday today, and I should be", "making him a lovely big birthday cake.")
    task.chatNpc("I've forgotten to buy the ingredients. I'll never get", "them in time now. He'll sack me! What will I do? I have", "four children and a goat to look after. Would you help", "me? Please?")
    when (task.options("Yes", "No")) {
        1 -> {
            task.chatPlayer("Yes, I'll help you.")
            task.player.startQuest(cooksAssistant)
            task.chatNpc("Oh thank you, thank you. I need milk, an egg and", "flour. I'd be very grateful if you can get them for me.")
            task.chatPlayer("So where do I find these ingredients then?")
            helpChat(task)
        }
        2 -> {
            task.chatPlayer("No, I don't feel like it. Maybe later.")
            task.chatNpc("Fine. I always knew you Adventurer types were callous", "beasts. Go on your merry way!")
        }
    }
}

suspend fun helpChat(task: QueueTask) {
    when (task.options("Where do I find some flour?", "How about milk?", "And eggs? Where are they found?", "Actually, I know where to find this stuff.")) {
        1 -> {
            task.chatNpc("There is a Mill fairly close, go North and then West.", "Mill Lane Mill is just off the road to Draynor. I", "usually get my flour from there.")
            task.chatNpc("Talk to Millie, she'll help, she's a lovely girl and a fine", "Miller. Make sure you take a pot with you for the flour.")
            helpChat(task)
        }
        2 -> {
            task.chatNpc("There is a cattle field on the other side of the river,", "just across the road from the Groat's Farm.")
            task.chatNpc("Talk to Gillie Groats, she looks after the Dairy cows - ", "she'll tell you everything you need to know about milking cows!")
...
suspend fun mainChat(task: QueueTask) {
    if(!task.player.finishedQuest(cooksAssistant)) {
        task.chatNpc("How are you getting on with finding the ingredients?")
        if(task.player.inventory.contains(Items.EXTRA_FINE_FLOUR) && task.player.inventory.contains(Items.TOPQUALITY_MILK) && task.player.inventory.contains(Items.SUPER_LARGE_EGG)) {
            ...
            task.chatNpc("I'm afraid not, only the big cheeses get to dine with the", "Duke.")
            task.chatPlayer("Well, maybe one day I'll be important enough to sit on", "the Duke's table.")
            task.chatNpc("Maybe, but I won't be holding my breath.")
            cooksAssistant.finishQuest(task.player)
        } else {
            task.chatPlayer("I haven't got them all yet, I'm still looking.")
            task.chatNpc("Please get the ingredients quickly - I'm running out of", "time! The Duke will throw me into the streets!")
        ...
        when (task.options("Do you have any other quests for me?", "I am getting strong and mighty.", "I keep on dying.", "Can I use your range?")) {
            1 -> {
            ...
            4 -> {
                ...
                task.chatNpc("Its called the Cook-o-matic 100. It uses a combination of", "state of the art temperature regulation and magic.")
