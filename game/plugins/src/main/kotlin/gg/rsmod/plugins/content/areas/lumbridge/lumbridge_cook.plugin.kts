...
suspend fun questStart(task: QueueTask) {
    ...
    task.chatNpc("Oh dear, oh dear, oh dear, I'm in a terrible terrible", "mess! It's the Duke's birthday today, and I should be", "making him a lovely big birthday cake.", facialExpression = FacialExpression.AFRAID)
    task.chatNpc("I've forgotten to buy the ingredients. I'll never get", "them in time now. He'll sack me! What will I do? I have", "four children and a goat to look after. Would you help", "me? Please?", facialExpression = FacialExpression.UPSET)
    when (task.options("Yes", "No")) {
        ...
        2 -> {
            ...
            task.chatNpc("Fine. I always knew you Adventurer types were callous", "beasts. Go on your merry way!", facialExpression = FacialExpression.ANGRY_2)
        }
    }
}
...
suspend fun mainChat(task: QueueTask) {
    if(!task.player.finishedQuest(cooksAssistant)) {
        ...
        if(task.player.inventory.contains(Items.EXTRA_FINE_FLOUR) && task.player.inventory.contains(Items.TOPQUALITY_MILK) && task.player.inventory.contains(Items.SUPER_LARGE_EGG)) {
            ...
            task.chatNpc("I'm afraid not, only the big cheeses get to dine with the", "Duke.", facialExpression = FacialExpression.UPSET)
            task.chatPlayer("Well, maybe one day I'll be important enough to sit at", "the Duke's table.", facialExpression = FacialExpression.CALM_TALK)
            ...
            task.chatNpc("Please get the ingredients quickly - I'm running out of", "time! The Duke will throw me into the streets!", facialExpression = FacialExpression.UPSET)
