    ...
    override fun getObjective(player: Player, stage: Int): QuestStage = when (stage) {
        ...
        4 -> QuestStage (
            objectives = listOf(
                ...
                striked("amulet when doing so."),
                "",
                if (player.inventory.contains(Items.MUDDY_SKULL)) "I should take the ${red("skull")} back to the ${red("ghost")} in the"
                else
                    "I should go and search the ${red("Mining Spot")} on the coast ${red("south")}",
                if (player.inventory.contains(Items.MUDDY_SKULL)) "graveyard so it can rest in peace."
                else
                    "${red("of Lumbridge")} for the ${red("ghost's skull.")}"
            )
        )
