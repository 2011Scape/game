    ...
    when (it.options(
        ...
        1 -> {
            ...
            if (it.player.inventory.hasFreeSpace()) {
                if (it.player.inventory.remove(Items.COINS_995, 2).hasSucceeded()) {
                    it.player.inventory.add(Items.BEER)
                    it.itemMessageBox("The bartender hands you a fine ale.", Items.BEER)
                } else {
                    it.chatNpc("You don't have enough coins in your inventory.",
        ...
        2 -> {
            ...
            when (it.options(
                ...
                2 -> {
                    ...
                    it.chatPlayer("Nope, still don't understand what you are talking about. What does 'online' mean?", wrap = true)
                    it.chatNpc("It's a sort of connection between magic boxes across the world, big boxes on" +
                            "people's desktops and little ones people can carry. They talk to each other to play games.", wrap = true)
                    it.chatPlayer("I give up. You're obviously completely mad!")
                }
                3 -> {
                    it.chatPlayer("Just a small clue?")
                    it.chatNpc("Go and talk to the bartender at the Jolly Boar Inn, he doesn't seem to mind giving away clues.", wrap = true)
                }
                4 -> {
                    it.chatPlayer("Do you know where I can get some good equipment?")
                    it.chatNpc("Well, there's the sword shop across the road, or there's also all sorts of shops up around the market.", wrap = true)
                }
            }
        }
        3 -> {
