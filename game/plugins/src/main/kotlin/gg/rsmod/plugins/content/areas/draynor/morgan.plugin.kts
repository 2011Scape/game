...
suspend fun preQuest(it: QueueTask) {
    ...
    when (
        ...
        1 -> { // Why don't you save your own village?
            ...
            it.chatNpc(
                "I don't know, I just moved here with my wife. We'd move on again, but we're down on our luck and can't afford to. ",
                facialExpression = FacialExpression.SAD,
                wrap = true,
            )
            it.chatNpc(
                "Besides, I don't want to abandon other innocents to this fate. This could be a good community, if only that vampyre would leave us.",
                wrap = true,
            )
        ...
        1 -> { // What do I need to do?
            ...
            when (it.options("How do I find Harlow?", "Right, I'll get on that.")) {
                1 -> { // How do I find Harlow?
                    ...
                    it.chatNpc(
                        "Leave Draynor Village heading north-east. Cross the River Lum and head north to Varrock.",
                        wrap = true,
                    )
                    it.chatNpc(
                        "Enter Varrock from the south and on your right should be a pub called the Blue Moon inn. Harlow should be in there: drinking, no doubt.",
                        wrap = true,
                    )
                }
...
suspend fun afterHarlow(it: QueueTask) {
    ...
    it.chatNpc(
        "You must forgive him. He believes the beer frees him from his memories, but it's really just " +
            "destroying him.",
        wrap = true,
    )
}
...
suspend fun afterKillingVampyre(it: QueueTask) {
    ...
    it.chatNpc(
        "Praise Saradomin! Bless you! We are all saved, the",
        "vampyre is slain!",
        facialExpression = FacialExpression.SHOCK,
    )
