...
on_command("add_item", Privilege.ADMIN_POWER) {
    ...
    tryWithUsage(player, args, "Invalid format! Example of proper command <col=42C66C>::add_item [id] [amount]</col>") { values ->
...
fun addItem(player: Player, itemId: Int, amount: Int) {
    val string =
 ...
 */
fun addNpc(player: Player, id: Int) {
    if (getName(id) == null) {
        player.message(
