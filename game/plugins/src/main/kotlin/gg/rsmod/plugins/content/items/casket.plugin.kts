...
on_item_option(item = Items.CASKET, option = "open") {
    if(player.inventory.remove(player.getInteractingItem(), beginSlot = player.getInteractingItemSlot()).hasSucceeded()) {
        val drop = DropTableFactory.createDropInventory(player, Items.CASKET, DropTableType.CHEST) ?: return@on_item_option
        val item = drop[0]
        player.queue {
            val name = world.definitions.get(ItemDef::class.java, item.id).name.lowercase()
            itemMessageBox("You open the casket. Inside you find ${formatWithIndefiniteArticle(name)}.", item = item.id, amountOrZoom = item.amount)
        }
    }
}

val table = DropTableFactory
val casketRewards = table.build {
    main {
        total(128)
