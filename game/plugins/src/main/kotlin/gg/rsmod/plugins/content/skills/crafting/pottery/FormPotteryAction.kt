...
object FormPotteryAction {
    ...
    suspend fun formPottery(task: QueueTask, data: PotteryItem, amount: Int) {
        ...
        repeat(maxAmount) {
            ...
            if (!inventory.remove(Items.SOFT_CLAY, amount = if (urn) 2 else 1, assureFullRemoval = true).hasSucceeded())
                return
            inventory.add(item = data.unfired, assureFullInsertion = true)
            player.addXp(Skills.CRAFTING, data.formingExperience)
            var resultName = player.world.definitions.get(ItemDef::class.java, data.fired).name.lowercase()
    ...
    private suspend fun canFormPottery(task: QueueTask, data: PotteryItem) : Boolean {
        val player = task.player
        val inventory = player.inventory
