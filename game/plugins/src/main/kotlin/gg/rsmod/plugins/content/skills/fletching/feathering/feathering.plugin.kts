...
fun featherShaft(
    ...
    val feather =
        feathers.firstOrNull { it == player.attr[INTERACTING_ITEM_ID] || it == player.attr[OTHER_ITEM_ID_ATTR] } ?: -1
    val featheredDef = featheringDefinitions[feathered] ?: return

    when (
        ceil(
            min(
                player.inventory.getItemCount(featheredDef.raw),
                (
                    player.inventory.getItemCount(feather) /
                        featheredDef.feathersRequired
                ),
            ) /
                featheredDef.amount.toDouble(),
        ).toInt()
    ) {
        0 -> return
...
fun feather(
    ...
    val feather =
        feathers.firstOrNull { it == player.getInteractingItemId() || it == player.getInteractingOtherItemId() } ?: -1
    val featheredDef = featheringDefinitions[feathered] ?: return
