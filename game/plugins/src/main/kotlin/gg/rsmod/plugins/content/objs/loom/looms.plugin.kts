...
LOOMS.forEach {
    on_obj_option(it, option = "weave") {
        ...
        val productNames = weavingData.map { data ->
            if (data == WeavingData.MILESTONE_CAPES)
                "Milestone capes"
            else player.world.definitions.get(ItemDef::class.java, data.resultItem).name
        }.toTypedArray()
        val resourceNames = weavingData.map { resources -> "(${resources.rawName})" }.toTypedArray()
        player.queue {
            produceItemBox(*products, option = SkillDialogueOption.MAKE, names = productNames, extraNames = resourceNames, logic = ::weaveItem)
        }
    }
}

fun weaveItem(player: Player, item: Int, amount: Int) {
    val def = WeavingData.weavingDefinitions[item] ?: return
    if (def == WeavingData.MILESTONE_CAPES) {
        val capes = intArrayOf(Items.MILESTONE_CAPE_10, Items.MILESTONE_CAPE_20, Items.MILESTONE_CAPE_30, Items.MILESTONE_CAPE_40, Items.MILESTONE_CAPE_50, Items.MILESTONE_CAPE_60, Items.MILESTONE_CAPE_70, Items.MILESTONE_CAPE_80, Items.MILESTONE_CAPE_90)
        val resources = capes.map {
            val name = player.world.definitions.get(ItemDef::class.java, it).name
...
fun weaveCape(player: Player, item: Int, amount: Int) {
