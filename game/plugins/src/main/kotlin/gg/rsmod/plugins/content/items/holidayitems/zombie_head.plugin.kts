...
on_item_option(Items.ZOMBIE_HEAD, option = "question") {
    player.queue {
        when(options("How did you die?", "What is your name?", "Can you do any tricks?", "Want a new hat?", "Want to go scare some people?")) {
            1 -> { // How did you die?
                ...
                chatNpc("I stuck my neck out for an old friend.", npc = Npcs.ZOMBIE_HEAD, facialExpression = FacialExpression.REALLY_SAD)
                chatPlayer("You shouldn't get so cut up about it.", facialExpression = FacialExpression.DISDAIN)
                chatNpc("Well if I keep it all bottled up I'll turn into a total head case.", npc = Npcs.ZOMBIE_HEAD, facialExpression = FacialExpression.REALLY_SAD, wrap = true)
            ...
            3 -> { // Can you do any tricks?
                ...
                chatNpc("Because I used to be able to do a handstand for over an hour while juggling cannon balls with my feet...", npc = Npcs.ZOMBIE_HEAD, facialExpression = FacialExpression.REALLY_SAD, wrap = true)
            ...
            4 -> { // Want a new hat?
                ...
                chatNpc("Because currently you wave me about by my hair, and it hurts.", npc = Npcs.ZOMBIE_HEAD, facialExpression = FacialExpression.REALLY_SAD)

            }
            5 -> { // Want to go scare some people?
                chatPlayer("Hey, Head?", facialExpression = FacialExpression.CONFUSED)
                chatNpc("Will you ever leave me alone?", npc = Npcs.ZOMBIE_HEAD, facialExpression = FacialExpression.MEAN_FACE)
                chatPlayer("No!", facialExpression = FacialExpression.CHEERFUL)
                chatPlayer("Want to go scare some people?", facialExpression = FacialExpression.CONFUSED)
                chatNpc("Let's leave it for now.", npc = Npcs.ZOMBIE_HEAD, facialExpression = FacialExpression.REALLY_SAD)
                ...
                chatNpc("If I try really hard I might be able to will myself", "deader...", npc = Npcs.ZOMBIE_HEAD, facialExpression = FacialExpression.WAILING)
            }
        }
    }
}