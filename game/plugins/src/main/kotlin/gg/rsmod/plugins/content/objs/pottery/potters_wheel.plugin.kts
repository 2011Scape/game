...
val POTTERS_WHEELS = listOf(Objs.POTTERS_WHEEL, Objs.POTTERS_WHEEL_20375, Objs.POTTERS_WHEEL_34801, Objs.POTTERS_WHEEL_3200, Objs.POTTERS_WHEEL_4310, Objs.POTTERS_WHEEL_5092, Objs.POTTERS_WHEEL_61335,
...
fun handlePottersWheel(player: Player) {
    ...
    val names = pottery.map {data ->
        if (data.fired == -1)
            "Urns"
        else player.world.definitions.get(ItemDef::class.java, data.fired).name
    }.toTypedArray()
...
fun formItem(player: Player, item: Int, amount: Int) {
    ...
    if (def == PotteryData.NOT_URNS && data.fired == -1) {
        val displayItems = listOf(Items.COOKING_URN_UNF, Items.FISHING_URN_UNF, Items.MINING_URN_UNF, Items.SMELTING_URN_UNF, Items.WOODCUTTING_URN_UNF, Items.ACCURSED_URN_UNF)
        val categories = listOf("Cooking urns", "Fishing urns", "Mining urns", "Smelting urns", "Woodcutting urns", "Prayer urns")
        val maxAmount = player.inventory.getItemCount(Items.SOFT_CLAY) / 2
        player.queue {
            produceItemBox(*displayItems.toIntArray(), title = "Which skill would you like to make urns for?", maxItems = maxAmount, names = categories.toTypedArray(), logic = ::pickUrnLevel)
        }
    } else {
...
fun pickUrnLevel(player: Player, item: Int, amount: Int) {
    ...
    val names = urnData.map { data -> player.world.definitions.get(ItemDef::class.java, data.unfired).name.dropLast(6) }
    val maxAmount = player.inventory.getItemCount(Items.SOFT_CLAY) / 2
    player.queue {
        produceItemBox(*products.toIntArray(), title = "Which urn would you like to make?", maxItems = maxAmount, names = names.toTypedArray(), logic = ::formUrn)
    }
}

fun formUrn(player: Player, item: Int, amount: Int) {
    player.queue { FormPotteryAction.formPottery(this, PotteryData.findPotteryItemByItem(item)!!, amount) }
}