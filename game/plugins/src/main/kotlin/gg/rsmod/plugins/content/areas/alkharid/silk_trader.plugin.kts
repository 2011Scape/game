...
suspend fun chat(it: QueueTask) {
    ...
    when (world.random(0)) {
        0 -> when (it.options("How much are they?", "No. Silk doesn't suit me.")) {
            1 -> {
...
suspend fun buySilk(it: QueueTask, price: Int) {
    if (it.player.inventory.getItemCount(Items.COINS_995) >= price) {
        it.player.inventory.remove(Items.COINS_995, price)
        if (it.player.inventory.freeSlotCount >= 1) it.player.inventory.add(Items.SILK, 1)
        else {
