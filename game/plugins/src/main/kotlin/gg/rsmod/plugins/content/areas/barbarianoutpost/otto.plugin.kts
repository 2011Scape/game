...
suspend fun barbarianTrainingDialogue(it: QueueTask) {
    ...
    val option = it.options("I really don't think I am related to any barbarian.",
                            "You think so?",
                            "Who are you?",
...
suspend fun youThinkSo(it: QueueTask) {
    ...
    suspend fun detailsCunningHarpoons(it: QueueTask) {
        ...
        it.chatNpc("Oh, my mind slipped for a moment, this method does",
            "indeed work with shark - though in this case the action",
            "must be more of a frenzied thrashing of the arm than a",
            "wriggle")
        it.chatPlayer("...and I thought Fishing was a safe way to pass the time.", facialExpression = FacialExpression.CONFLICTED)
    ...
    suspend fun detailsHerblore(it: QueueTask) {
        if (it.player.hasItem(Items.ATTACK_MIX_2)) {
            it.chatPlayer("What was that secret knowledge of Herblore we talked",
                "of?")
            it.chatNpc("I see you have my potion. I will say no more than that",
                                "I am eternally grateful.")
            it.chatPlayer("I feel I am missing some vital information about your",
                                    "need for this potion, though I often have this suspicion.")
            it.chatNpc("I will not reveal all of my private matters to you. Some",
...
suspend fun iSeekMoreAnswers(it: QueueTask) {
    if (it.player.attr.has(learnedBarbarianHarpooning) && it.player.attr.has(learnedBarbarianFiremaking) && (it.player.attr.has(
            learnedBarbarianRod) && it.player.attr.has(learnedBarbarianHerblore))) {
        whoAreYou(it)
    } else {
    ...
    suspend fun whoAreYou(it: QueueTask) {
        ...
        it.chatNpc("I am Otto Godblessed, the last of the Godblessed line.",
