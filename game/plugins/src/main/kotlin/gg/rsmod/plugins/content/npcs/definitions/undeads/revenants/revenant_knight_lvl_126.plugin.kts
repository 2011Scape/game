...
val knight =
    table.build {
        main {
            ...
            // Least rare, but still rare 1/1.8k
            ...
            val takenSlots = 1137
            val remaining = 82_500 - takenSlots
            obj(Items.COINS_995, quantityRange = 1..545, remaining)
        }
    }

table.register(knight, id)
...
on_npc_spawn(id) {
    npc.animate(Anims.REVENANT_KNIGHT_SPAWN)
}
...
set_combat_def(id) {
    ...
    anims {
        attack = Anims.REVENANT_KNIGHT_MELEE_ATTACK
        block = Anims.REVENANT_KNIGHT_BLOCK
        death = Anims.REVENANT_KNIGHT_DEATH
    }
    aggro {
        radius = 5
    }
