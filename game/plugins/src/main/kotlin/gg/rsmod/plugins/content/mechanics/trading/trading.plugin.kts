...
on_player_option(option = "Trade with") {
    ...
    if (!requests.contains(partner)) {
        ...
        partner.message(TRADE_REQ_STRING, ChatMessageType.TRADE_REQ, Misc.formatForDisplay(player.username))
    } else {
    ...
    player.getTradeSession()?.let { trade ->
        ...
        player.queue(TaskPriority.WEAK) {
            ...
            val amount = when (opt) {
                ...
                64 -> 5
                4 -> 10
                52 -> inventory.getItemCount(item.id)
    ...
    player.getTradeSession()?.let { trade ->
        ...
        player.queue(TaskPriority.WEAK) {
            ...
            val amount = when (opt) {
                ...
                64 -> 5
                4 -> 10
                52 -> container.getItemCount(item.id)
                81 -> inputInt("Enter amount:")
                else -> 1
            }
...
// Accept buttons
on_button(TRADE_INTERFACE, 16) { player.getTradeSession()?.progress() }
on_button(ACCEPT_INTERFACE, 21) { player.getTradeSession()?.progress() }

// Decline buttons
on_button(TRADE_INTERFACE, 18) { player.getTradeSession()?.decline() }
on_button(ACCEPT_INTERFACE, 22) { player.getTradeSession()?.decline() }
