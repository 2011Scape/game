...
on_obj_option(obj = Objs.WELL_3485, option = "search") {
    player.queue{
        doubleMessageBox("You look down the well and see the filthy polluted water", "of the River Salve moving slowly along.")
    }
}
...
on_obj_option(obj = Objs.GATE_3444, option = "open") {
    if(player.getCurrentStage(quest = PriestInPeril) >= 5) {
        ...
        player.queue {
            messageBox("The door is securely locked from inside.")
            chatPlayer("Hmmm... From the looks of things, it seems as though",
...
//Grave Monuments
val graveMonuments = intArrayOf(Objs.MONUMENT, Objs.MONUMENT_3494, Objs.MONUMENT_3495, Objs.MONUMENT_3496,
    Objs.MONUMENT_3497, Objs.MONUMENT_3498, Objs.MONUMENT_3499)
...
graveMonuments.forEach {
    on_obj_option(it, option = "take-from") {
        ...
        player.dealHit(target = player, minHit = takeDamage, maxHit = takeDamage+0.01, landHit = true, delay = 1, hitType = HitType.REGULAR_HIT)
