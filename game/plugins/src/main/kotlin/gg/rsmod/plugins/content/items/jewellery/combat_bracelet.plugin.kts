...
val COMBAT_BRACELET = intArrayOf(
    Items.COMBAT_BRACELET_1, Items.COMBAT_BRACELET_2, Items.COMBAT_BRACELET_3, Items.COMBAT_BRACELET_4
)
...
private val LOCATIONS = mapOf(//TODO CHANGE TO CORRECT COORDS
    "Champions' Guild" to Tile(3086, 3503, 0),
    "Warriors' Guild" to Tile(2917, 3175, 0),
    "Monastery" to Tile(3104, 3249, 0),
    "Ranging Guild" to Tile(3293, 3162, 0),
)

COMBAT_BRACELET.forEach { item ->
    on_item_option(item = item, option = "Rub") {
        player.queue {
            when (options("Champions' Guild.", "Warriors' Guild.", "Monastery.", "Ranging Guild.", "Nowhere.")) {
                1 -> player.teleport(LOCATIONS["Champions' Guild"]!!, isEquipped = false)
                2 -> player.teleport(LOCATIONS["Warriors' Guild"]!!, isEquipped = false)
...
fun message(original: Int): String {
    return when (original) {
        ...
        else -> "Your combat bracelet has ran out of charges."
    }
}
