...
val ork =
    table.build {
        main {
            ...
            // Least rare, but still rare 1/2k
            ...
            val takenSlots = 1137
            val remaining = 90_915 - takenSlots
            obj(Items.COINS_995, quantityRange = 1..495, remaining)
        }
    }

table.register(ork, id)
...
on_npc_spawn(id) {
    npc.animate(Anims.REVENANT_ORK_SPAWN)
}
...
set_combat_def(id) {
    ...
    anims {
        attack = Anims.REVENANT_ORK_MELEE_ATTACK
        block = Anims.REVENANT_ORK_BLOCK
        death = Anims.REVENANT_ORK_DEATH
    }
    aggro {
        radius = 5
    }
