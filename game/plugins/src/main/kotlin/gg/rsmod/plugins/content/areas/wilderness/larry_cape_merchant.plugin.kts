...
val LARRY = Npcs.LARRY

create_shop("Larry's Wilderness Cape Shop", currency = CoinCurrency(), purchasePolicy = PurchasePolicy.BUY_STOCK, containsSamples = false) {
    items[0] = ShopItem(Items.TEAM3_CAPE, amount = 10)
    items[1] = ShopItem(Items.TEAM13_CAPE, amount = 10)
    items[2] = ShopItem(Items.TEAM23_CAPE, amount = 10)
    items[3] = ShopItem(Items.TEAM33_CAPE, amount = 10)
    items[4] = ShopItem(Items.TEAM43_CAPE, amount = 10)
}

on_npc_option(npc = LARRY, option = "trade") {
    player.openShop("Larry's Wilderness Cape Shop")
}

on_npc_option(npc = LARRY, option = "talk-to") {
    player.queue {
        ...
        when(options("What do team capes do?", "Yes please!", "No thanks.")) {
            1 -> {
                ...
                when (options("Yes please!", "No thanks.")) {
                    1 -> player.openShop("Larry's Wilderness Cape Shop")
                    2 -> {
                        chatPlayer("No thanks.")
                    }
                }
            }
            2 -> player.openShop("Larry's Wilderness Cape Shop")
