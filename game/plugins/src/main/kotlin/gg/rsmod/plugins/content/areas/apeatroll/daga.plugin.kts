        ...
        player.queue {
            ...
            when (options("Yes please.", "No, thanks", "Do you have any Dragon Scimitars in stock?")) {
                ...
                THIRD_OPTION -> {
                    ...
                    chatNpc("It just so happens I recently got a fresh delivery",
                        "do you want to buy one?", facialExpression = FacialExpression.OLD_NORMAL)
                    when (options("Yes", "No")) {
                        FIRST_OPTION -> {
                            chatPlayer("Yes, please.")
                            if (player.inventory.hasFreeSpace()) {
                                if (player.inventory.remove(Items.COINS_995, 100000).hasSucceeded()) {
                                    ...
                                    chatNpc("Sorry but you don't have enough to buy one",
                                        "at the moment it costs 100000 gold coins.", facialExpression = FacialExpression.OLD_NORMAL)
                                }
                            } else {
                                chatNpc("Sorry but you don't have enough space in your inventory.", facialExpression = FacialExpression.OLD_NORMAL)
                            }
                        }
